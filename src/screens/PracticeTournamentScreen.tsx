...
export const PRACTICE_MATCH_CONFIG: RoundConfig = {
  categoryId: 'mixed',
 ...
 */
export default function PracticeTournamentScreen({ onBack, onMatchPlayed }: Props) {
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.rivalBadge} pointerEvents="none">
          ...
          <Text style={styles.rivalBadgeText}>
            {t('practice.roundName', roundLabel(bracket.round, t))} · {rival?.name ?? '—'}
          </Text>
        </View>
      </View>
    );
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <SoundTouchable onPress={onBack} style={styles.back} accessibilityRole="button">
        <Text style={styles.backText}>{`‹ ${t('practice.back')}`}</Text>
      </SoundTouchable>
...
function roundLabel(round: number, t: ReturnType<typeof useT>): string {
  if (round === TOURNAMENT_ROUNDS - 1) return t('practice.final');
  if (round === TOURNAMENT_ROUNDS - 2) return t('practice.semifinal');
...
function Intro({
  ...
  return (
    <>
      ...
      <Text style={styles.lead}>{t('practice.intro')}</Text>

      <View style={styles.rulesCard}>
        {[t('practice.rule1'), t('practice.rule2'), t('practice.rule3')].map((rule) => (
      ...
      <SoundTouchable style={styles.primaryButton} onPress={onStart} activeOpacity={0.88}>
        <Text style={styles.primaryButtonText}>{t('practice.start')}</Text>
      </SoundTouchable>
    </>
  );
}
...
function roundName(rounds: number, t: ReturnType<typeof useT>): string {
  if (rounds >= TOURNAMENT_ROUNDS) return t('practice.bestTitle');
    ...
    <>
        ...
        <Text style={styles.lead}>
          {iWon
            ...
            : bracket.myExitRound !== null
              ? t('practice.youLost', roundLabel(bracket.myExitRound, t))
              : t('practice.finished')}
          {champion && !iWon ? `\n${t('practice.champion', champion.name)}` : ''}
        </Text>
      ) : (
        <Text style={styles.lead}>{t('practice.alive', survivorsAt(bracket, bracket.round))}</Text>
      )}

      {!over && rival && (
        <View style={styles.nextCard}>
          <Text style={styles.nextLabel}>{t('practice.nextMatch', roundLabel(bracket.round, t))}</Text>
          <View style={styles.rivalRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.rivalKind}>
                {rival.kind === 'bot' ? t('practice.opponentBot') : t('practice.opponentPlayer')}
              </Text>
            </View>
          </View>
          <SoundTouchable style={styles.primaryButton} onPress={onPlay} activeOpacity={0.88}>
            <Text style={styles.primaryButtonText}>{t('practice.play', MATCH_QUESTIONS)}</Text>
          </SoundTouchable>
        </View>
      )}
      ...
      {over ? (
        <SoundTouchable style={styles.primaryButton} onPress={onRestart} activeOpacity={0.88}>
          <Text style={styles.primaryButtonText}>{t('practice.again')}</Text>
        </SoundTouchable>
      ) : (
        <SoundTouchable style={styles.ghostButton} onPress={onAbandon} activeOpacity={0.85}>
          <Text style={styles.ghostButtonText}>{t('practice.abandon')}</Text>
        </SoundTouchable>
      )}
    </>
  );
    ...
    <View style={[styles.pathRow, round > 0 && styles.pathRowDivided]}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.pathSubtitle} numberOfLines={1}>
          {rival ? rival.name : bracket.myExitRound !== null && round > bracket.myExitRound ? t('practice.outAlready') : t('practice.pending')}
        </Text>
      </View>
      {decided && (
  ...
  return (
    <>
      ...
      <Text style={styles.title}>
        {champion ? t('practice.youWon') : won ? t('practice.matchWon') : t('practice.matchLost')}
      </Text>

      <View style={styles.scoreCard}>
        <View style={styles.scoreSide}>
          <Text style={styles.scoreName}>{t('practice.you')}</Text>
      ...
      <Text style={styles.lead}>
        {champion
          ? t('practice.championLead')
          : won
            ? t('practice.advanced', roundLabel(bracket.round, t), survivorsAt(bracket, bracket.round))
            : t('practice.eliminated', roundLabel(round, t))}
      </Text>

      <SoundTouchable style={styles.primaryButton} onPress={onContinue} activeOpacity={0.88}>
        <Text style={styles.primaryButtonText}>{t('practice.toBracket')}</Text>
      </SoundTouchable>
    </>
  );
}
